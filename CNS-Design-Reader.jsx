...
const Arch = () => (
  <div className="overflow-x-auto">
    <svg width="950" height="352" viewBox="0 0 960 352" style={{ minWidth: 900 }}>
      ...
      <Box x={660} y={26} w={62} h={26} l1="inbox-q" tone="queue" />
      <Box x={660} y={70} w={62} h={26} l1="email-q" tone="queue" />
      <Box x={660} y={114} w={62} h={26} l1="slack-q" tone="queue" />
      <Box x={742} y={16} w={110} h={46} l1="Inbox λ" l2="render · DDB write" />
      <Box x={866} y={16} w={88} h={46} l1="AppSync" l2="GraphQL + subs" />
      <Box x={742} y={70} w={110} h={38} l1="Email λ" l2="render · outbox · SES" />
      <Box x={742} y={116} w={110} h={38} l1="Slack λ" l2="render · token bucket" />
      ...
      <Arrow d="M614 78 L656 41" mid="mA" />
      <Arrow d="M614 88 L656 84" mid="mA" label="filter: channels[]" lx={630} ly={62} />
      <Arrow d="M614 98 L656 128" mid="mA" />
      <Arrow d="M722 39 H738" mid="mA" />
      <Arrow d="M722 83 H738" mid="mA" />
      <Arrow d="M722 127 H738" mid="mA" />
      <Arrow d="M852 39 H862" mid="mA" label="publish" lx={857} ly={30} />
    ...
    <div className="mt-1 text-xs" style={{ color: C.sub }}>
      ...
      filter policy. AI types are published <span style={mono}>stage=enrich</span> — only the enrich queue matches, so <b>no channel can see the message
      until the Enricher republishes it as <span style={mono}>stage=deliver</span></b>. High-res diagrams: diagrams/ (v14_components.png, f1–f6).
    </div>
  </div>
);
...
const entities = [
  ...
  ["AuditEvent", "auditId = notifId#stage#attempt (idempotent) · stage · outcome · reason — no payload, recipient as HMAC token; new in v1.4: enrich_waiting · enrich_retry · enrich_dlq · redriven"],
  ...
  return (
    <div className="min-h-screen pb-16" style={{ background: C.paper, color: C.ink, ...disp }}>
      <header className="px-4 pt-6" style={{ background: C.navy }}>
        <div className="mx-auto max-w-3xl pb-4 text-white">
          ...
          <p className="mt-1 text-sm" style={{ color: "#C9D8E6" }}>
            One simple serverless pipeline — Intake λ → SNS attribute routing → one λ per channel — with AppSync as the entire in-app
            surface and an AI gate that holds enriched notifications until the model has finished. Companion to docs/CNS-System-Design-v1.4.md.
          </p>
          <Pipeline />
        </div>
      </header>
      ...
      <main className="mx-auto max-w-3xl space-y-3 px-3 pt-3">
        ...
        <Section id="flows" title="Sequence flows" kicker="5 flows · sources in diagrams/*.mmd (rendered PNGs alongside)">
